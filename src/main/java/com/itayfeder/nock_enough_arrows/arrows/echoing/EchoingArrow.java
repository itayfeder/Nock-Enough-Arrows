...
public class EchoingArrow extends AbstractArrow {
    ...
    public void tick() {
        ...
        if (this.level.isClientSide && !this.inGround) {
            this.level.addParticle(ParticleTypes.BUBBLE_POP, this.getX(), this.getY(), this.getZ(), 0.0D, 0.0D, 0.0D);
        }
    }
    ...
    protected void onHitBlock(BlockHitResult p_36755_) {
        ...
        for (LivingEntity entity : entities) {
            if (entity != null && entity != this.getOwner()) {
                ...
                this.level.addParticle(new VibrationParticleOption(new VibrationPath(p_36755_.getBlockPos().above(), new EntityPositionSource(entity.getId()), 10)), this.getX(), this.getY(), this.getZ(), 0.0D, 0.0D, 0.0D);
            }
        }
    }
}
